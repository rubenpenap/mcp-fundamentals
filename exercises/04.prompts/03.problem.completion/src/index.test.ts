...
test('Prompt Argument Completion', async () => {
	...
	try {
		...
		const list = await client.listPrompts()
		invariant(list.prompts.length > 0, '🚨 No prompts found - need prompts to test completion')
		
		const firstPrompt = list.prompts[0]
		invariant(firstPrompt, '🚨 No prompts available to test completion')
		invariant(firstPrompt.arguments && firstPrompt.arguments.length > 0, '🚨 Prompt should have completable arguments')
		...
		invariant(Array.isArray(completionResult.completion?.values), '🚨 Prompt completion should return an array of values')
		invariant(completionResult.completion.values.length > 0, '🚨 Prompt completion should return at least one matching result for value="1"')
		...
		console.error('🚨 This exercise teaches you how to add completion support to prompt arguments')
		console.error('🚨 You need to:')
		console.error('🚨   1. Add "completion" to your server capabilities')
		console.error('🚨   2. Import completable from @modelcontextprotocol/sdk/server/completable.js')
		console.error('🚨   3. Wrap your prompt argument schema with completable():')
		console.error('🚨      entryId: completable(z.string(), async (value) => { return ["1", "2", "3"] })')
		console.error('🚨   4. The completion callback should filter entries matching the partial value')
		console.error('🚨   5. Return an array of valid completion strings')
		console.error(`🚨 Error details: ${error?.message || error}`)
		
		if (error?.code === -32601) {
			throw new Error('🚨 Completion capability not declared - add "completion" to server capabilities and use completable() for prompt arguments')
		} else if (error?.code === -32602) {
			throw new Error('🚨 Completable arguments not implemented - wrap prompt arguments with completable() function')
		} else {
			throw new Error(`🚨 Prompt argument completion not working - check capability declaration and completable() usage. ${error}`)
		}
	}
})
